    ...
    public class Module_softDAL : Module_articleDAL
    {
        ...
        public override string GetContent(string _ChannelId, string _ContentId, int _CurrentPage)
        {
            using (DbOperHandler _doh = new Common().Doh())
            {
                ...
                dtContent.Clear();
                dtContent.Dispose();
                return ContentList[0].ToString().Replace("{$SoftUrl}", ContentList[1].ToString());
            }
        ...
        {
            //下载地址分割
            string SoftUrl = dt.Rows[0]["SoftUrl"].ToString().Replace("\r\n", "\r");
            string[] _SoftUrl = SoftUrl.Split(new string[] { "\r" }, StringSplitOptions.None);
            string TempStr = string.Empty;
            for (int j = 0; j < _SoftUrl.Length; j++)
            {
                string _url = _SoftUrl[j];
                ...
                if (_thisURL.StartsWith("http://") || _thisURL.StartsWith("https://") || _thisURL.StartsWith("ftp://"))
                    ...
                    TempStr += "&nbsp;&nbsp;<a class=\"iframe\" href=\"" + site.Url + site.Dir + "plus/download.aspx?ChannelType=soft&ChannelId=" + dt.Rows[0]["ChannelId"].ToString() + "&id=" + dt.Rows[0]["Id"].ToString() + "&NO=" + j + "\">" + _thisTXT + "</a>";
            }
