...
function ajaxCheck(ordernum){
	...
	$.ajax({
		...
		url:		"userorder_ajax.aspx?oper=ajaxCheck&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){top.JumboTCMS.Loading.hide();alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function ajaxDel(ordernum){
	$.ajax({
		...
		dataType:	"json",
		data:		"ordernum="+ordernum,
		url:		"userorder_ajax.aspx?oper=ajaxDel&clienttime="+Math.random(),
...
<tbody>
	...
	<tr>
		...
		<td align="center">
			...
			{#if $T.record.state == "0"}
			<a href="javascript:void(0);" onclick="top.JumboTCMS.Confirm('确定要作废该订单?', 'getCurrentIframe().ajaxDel(\'{$T.record.ordernum}\')');">作废订单</a>
			{#else}
			<font color='#cccccc'>作废订单</font>
			{#/if}
			{#if $T.record.state == "0" || $T.record.state == "1"}
           		<a href="javascript:void(0);" onclick="top.JumboTCMS.Confirm('你确定已收到对方汇款/转账，并准备发货吗?', 'getCurrentIframe().ajaxCheck(\'{$T.record.ordernum}\')');">准备发货</a>
			{#else}
