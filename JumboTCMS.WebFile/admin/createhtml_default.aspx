...
function getTreeJSON(){
	$.getJSON("class_ajax.aspx?oper=ajaxTreeJson&ccid=<%=ChannelId%>&clienttime="+Math.random(), {}, function(treedata) {
		var o = {
			showcheck: true,
			...
			oncount: function() {
				...
				if (_val != null){
					...
					$('#ClassCount').html('选择数量：0');
                        ...
                        url: "class_ajax.aspx?oper=ajaxTreeJson&&ccid=<%=ChannelId%>&clienttime="+Math.random()
		};
		o.data = treedata;
		$("#tree").treeview(o);//使用
	}); 
...
{
    JumboTCMS.Loading.show("正在生成频道页,请等待...");
	$.ajax({
		type:		"get",
		dataType:	"json",
...
function ajaxCreateByClass(Classs,act){
	...
	$.ajax({
		type:		"get",
		dataType:	"json",
		data:		"classid="+thisclasss+"&clienttime="+Math.random(),
...
function ajaxCreateById()
{
	...
	$.ajax({
		type:		"get",
		dataType:	"json",
		data:		"id1="+$("#txtId1").val()+"&id2="+$("#txtId2").val()+"&clienttime="+Math.random(),
...
.icon_ready{
	...
	overflow:hidden;
	text-indent:-999em;	
	background:url(../statics/admin/images/ready.gif) no-repeat;
}
.icon_loading{
	float:left;
	width:24px;
	...
	overflow:hidden;
	text-indent:-999em;	
	background:url(../statics/admin/images/loading.gif) no-repeat;
}
.icon_ignore{
	float:left;
	width:24px;
	...
	overflow:hidden;
	text-indent:-999em;	
	background:url(../statics/admin/images/ignore.gif) no-repeat;
}
.icon_failure{
	float:left;
	width:24px;
	...
	overflow:hidden;
	text-indent:-999em;	
	background:url(../statics/admin/images/failure.gif) no-repeat;
}
.icon_success{
	float:left;
	width:24px;
	...
	overflow:hidden;
	text-indent:-999em;	
	background:url(../statics/admin/images/success.gif) no-repeat;
}
</style>
</head>
<body>
