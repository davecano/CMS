...
function operater(act){
	...
	$.ajax({
		...
		dataType:	"json",
		data:		"ids="+ids,
		url:		"pointscard_ajax.aspx?oper=ajaxBatchOper&act="+act+"&clienttime="+Math.random(),
...
function ajaxDel(id){
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id,
		url:		"pointscard_ajax.aspx?oper=ajaxDel&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function ConfirmOffer(id){
	top.JumboTCMS.Confirm("确定要标识此充值卡并让它生效吗?", "getCurrentIframe().ajaxOffer("+id+")");
}
function ConfirmBindUser(id){
	top.JumboTCMS.Confirm("确定要创建一个新用户并捆绑充值卡吗?", "getCurrentIframe().ajaxBindUser("+id+")");
}
function ajaxOffer(id){
	...
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id,
		url:		"pointscard_ajax.aspx?oper=ajaxOffer&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){top.JumboTCMS.Loading.hide();alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function ajaxBindUser(id){
	...
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id,
		url:		"pointscard_ajax.aspx?oper=ajaxBindUser&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){top.JumboTCMS.Loading.hide();alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
