...
$(document).ready(function(){
	...
	$("#txtTitle").formValidator({tipid:"tipTitle",onshow:"请输入单页名称",onfocus:"请输入4-30个字符(2-15个汉字)"}).InputValidator({min:4,max:30,onerror:"请输入4-30个字符(2-15个汉字)"})
		...
		type : "get",
		data:		"id=<%=id%>",
		url:		"page_ajax.aspx?oper=checkname&clienttime="+Math.random(),
		datatype : "json",
		success : function(d){	
			if(d.result == "1")
				return true;
	...
	$("#txtOutUrl").formValidator({tipid:"tipOutUrl",onshow:"静态文件路径，不输入即为默认。如/aa/bb/cc.html",onfocus:"动态频道只支持aspx结尾，请慎重输入"}).RegexValidator({regexp:"^\(/[_\-a-zA-Z0-9\.]+(/[_\-a-zA-Z0-9\.]+)*\.(aspx|htm(l)?|shtm(l)?))$",onerror:"格式错误"});
...
<form id="form1" runat="server" onsubmit="return CheckFormSubmit()">
	<table class="formtable mrg10T">
		...
		<tr>
			<th> 模板文件 </th>
			<td> <% = site.Dir%>themes/<asp:TextBox ID="txtSource" runat="server" Width="250px" CssClass="ipt"></asp:TextBox>
