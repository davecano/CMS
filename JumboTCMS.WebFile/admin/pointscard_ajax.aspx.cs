...
namespace JumboTCMS.WebFile.Admin
{
    public partial class _pointscard_ajax : JumboTCMS.UI.AdminCenter
    {
        ...
        private void ajaxGetList()
        {
            ...
            totalCount = doh.Count("jcms_normal_pointscard");
            sqlStr = JumboTCMS.Utils.SqlHelper.GetSql0("A.id as id,A.CardNumber as CardNumber,A.CardPassword as CardPassword,B.UserName as UserName,A.Points as points,A.LimitedDate as LimitedDate,A.State as State,A.ActiveTime as ActiveTime,A.ActiveIP as ActiveIP", "jcms_normal_pointscard", "jcms_normal_user", "Id", PSize, page, "desc", joinStr, whereStr1, whereStr2);
            doh.Reset();
            doh.SqlCmd = sqlStr;
        ...
        private void ajaxBatchAdd()
        {
            ...
            if (_cardCount > 0 && _cardCount <= 100)
            {
                ...
                this._response = "alert('" + _cardCount + "张充值卡添加成功。');top.getCurrentIframe().ajaxList(1);top.JumboTCMS.Popup.hide();";
            }
            else
            {
        ...
        ///E:/swf/ </summary>
        ///E:/swf/ <param name="_act">操作类型{pass=审核,nopass=未审}</param>
