...
namespace JumboTCMS.WebFile.Admin
{
    public partial class _review_ajax : JumboTCMS.UI.AdminCenter
    {
        ...
        private void ajaxGetList()
        {
            ...
            if (id != "0") whereStr += " AND [ContentId]=" + id;
            if (ccid != "0") whereStr += " AND [ChannelId]=" + ccid;
            doh.Reset();
            doh.ConditionExpress = whereStr;
            totalCount = doh.Count("jcms_normal_review");
            sqlStr = JumboTCMS.Utils.SqlHelper.GetSql0("Id,IP,UserName,AddDate,Content,IsPass", "jcms_normal_review", "id", PSize, page, "desc", whereStr);
            doh.Reset();
            doh.SqlCmd = sqlStr;
