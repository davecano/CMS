<%@ Page Language="C#" AutoEventWireup="True" CodeBehind="forbidip_list.aspx.cs" Inherits="JumboTCMS.WebFile.Admin._forbidip_list" %>
...
function ajaxList(currentpage)
{
	...
	$.ajax({
		...
		url:		"forbidip_ajax.aspx?oper=ajaxGetList",
...
function operater(act){
	...
	$.ajax({
		...
		dataType:	"json",
		data:		"ids="+ids,
		url:		"forbidip_ajax.aspx?oper=ajaxBatchOper&act="+act+"&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){top.JumboTCMS.Loading.hide();alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function ajaxDel(id){
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id,
		url:		"forbidip_ajax.aspx?oper=ajaxDel&clienttime="+Math.random(),
...
<body>
    <div class="topnav">
        ...
        <ul id="topnavbar">
            <li class="topmenu"><a href="javascript:void(0);" onclick="top.JumboTCMS.Popup.show('forbidip_edit.aspx?id=0',500,320,false)" id="operater2" class="top_link"><span>添加黑名单</span></a>
            </li>
        </ul>
...
<thead>
	<tr>
		...
		<th scope="col" width="*">开始IP</th>
		<th scope="col" width="*">结束IP</th>
		<th scope="col" style="width:140px;">截至日期</th>
...
<tbody>
	...
	<tr>
		...
		<td align="left">{$T.record.expiredate}</td>
		...
		<td align="center">
           		<a href="javascript:void(0);" onclick="top.JumboTCMS.Popup.show('forbidip_edit.aspx?id={$T.record.id}',500,320,false)">修改</a> <a href="javascript:void(0);" onclick="ConfirmDel({$T.record.id})">删除</a>
		</td>
	</tr>
	{#/for}
</tbody>
