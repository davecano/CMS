...
function operater(act){
	...
	$.ajax({
		...
		dataType:	"json",
		data:		"ids="+ids,
		url:		"channel_ajax.aspx?oper=ajaxBatchOper&act="+act+"&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function move(id,isUp){
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id+"&up="+isUp,
		url:		"channel_ajax.aspx?oper=move&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function ajaxDel(id){
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id,
		url:		"channel_ajax.aspx?oper=ajaxDel&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
function ajaxUpdateUrlRewriter(){
	$.ajax({
		...
		dataType:	"json",
		data:		"",
		url:		"channel_ajax.aspx?oper=ajaxUpdateUrlRewriter&clienttime="+Math.random(),
		error:		function(XmlHttpRequest,textStatus, errorThrown){alert(XmlHttpRequest.responseText); },
		success:	function(d){
			switch (d.result)
			{
...
<thead>
	<tr>
		...
		<th scope="col" style="width:60px;">语言包</th>
		...
		<th scope="col" style="width:60px;">允许检索</th>
...
<tbody>
	...
	<tr>
		...
		<td align="center">{$T.record.languagecode}</td>
		<td align="center">
			{$T.record.classdepth}
		</td>
		<td align="left">
		...
		<td align="center" class="oper">
           	<a href="javascript:void(0);" onclick="JumboTCMS.Popup.show('channel_edit.aspx?ccid={$T.record.id}',-1,-1,true)">修改</a>
           	<a href="javascript:void(0);" onclick="ConfirmDel({$T.record.id})">删除</a>
           	<a href="<%=site.Dir %>{$T.record.dir}/default.aspx" target="_blank">浏览</a>
		</td>
	</tr>
	{#/for}
</tbody>
