<%@ Page Language="C#" AutoEventWireup="True" Codebehind="member_profile.aspx.cs" Inherits="JumboTCMS.WebFile.User._member_profile" %>
...
$(document).ready(function(){
	...
	$("#txtNickName")
		.formValidator({tipid:"tipNickName",onshow:"2-6个汉字",onfocus:"2-6个汉字"})
		.InputValidator({min:4,max:12,onerror:"2-6个汉字"});
	$("#txtEmail")
		.formValidator({tipid:"tipEmail",onshow:"请输入常用的私人邮箱",onfocus:"请输入常用的私人邮箱"})
		.InputValidator({min:8,max:50,onerror:"邮箱非法,请确认"})
		.RegexValidator({regexp:"email",datatype:"enum",onerror:"格式不正确"})
		.AjaxValidator({
		type : "get",
		url:		"../passport/ajax.aspx?oper=checkemail&id=<%=UserId%>&clienttime="+Math.random(),
		datatype : "json",
		success : function(d){	
			if(d.result == "1")
				return true;
	...
	$("#txtTrueName")
		.formValidator({tipid:"tipTrueName",onshow:"2-6个汉字",onfocus:"2-6个汉字"})
		.InputValidator({min:4,max:12,onerror:"2-6个汉字"})
		.RegexValidator({regexp:"chinese",datatype:"enum",onerror:"咱中国人就用纯的中文呗"});
	$("#txtIDCard")
		.formValidator({empty:true,tipid:"tipIDCard",onshow:"认真填写，填完不可修改",onfocus:"认真填写，填完不可修改"})
		.RegexValidator({regexp:"^([0-9a-zA-Z]{6,30})$",onerror:"6-30位数字,部分含有字母"});
	$("#txtWorkUnit")
		.formValidator({empty:true,tipid:"tipWorkUnit",onshow:"",onfocus:"比如：将博科技"})
		.InputValidator({min:1,onerror: "必填项!"});
	$("#txtAddress")
		.formValidator({empty:true,tipid:"tipAddress",onshow:"",onfocus:"比如：北京市某某区某某路"})
		.InputValidator({min:1,onerror: "必填项!"});
	$("#txtZipCode").formValidator({empty:true,tipid:"tipZipCode",onshow:"请输入邮编,可以为空",onfocus:"请输入邮编,可以为空"}).InputValidator({min:6,max:6,onerror:"邮政编码为6位,请确认"}).RegexValidator({regexp:"zipcode",datatype:"enum",onerror:"格式不正确"});
	$("#txtMobileTel").formValidator({empty:true,tipid:"tipMobileTel",onshow:"请输入你的手机号码",onfocus:"要填必须填正确"}).InputValidator({min:11,max:11,onerror:"必须是11位的,请确认"}).RegexValidator({regexp:"mobile",datatype:"enum",onerror:"不是正确的手机号码"});
	$("#txtTelephone").formValidator({empty:true,tipid:"tipTelephone",onshow:"请输入你的联系电话",onfocus:"要填必须填正确",onempty:"真的不想留电话了吗？"}).RegexValidator({regexp:"^[[0-9]{3}-|\[0-9]{4}-]?([0-9]{8}|[0-9]{7})?$",onerror:"格式例如：010-88666688"});
	$("#txtQQ").formValidator({empty:true,tipid:"tipQQ",onshow:"请输入你的QQ号,可以为空",onfocus:"请输入你的QQ号,可以为空",onempty:"你真的不想留QQ吗？"}).InputValidator({min:5,max:14,onerror:"位数不正确,请确认"}).RegexValidator({regexp:"qq",datatype:"enum",onerror:"格式不正确"});
	$("#txtMSN").formValidator({empty:true,tipid:"tipMSN",onshow:"请输入你的MSN帐号,可以为空",onfocus:"请输入你的MSN帐号,可以为空",onempty:"你真的不想留MSN吗？"}).RegexValidator({regexp:"email",datatype:"enum",onerror:"格式不正确"});
});
        ...
        <form id="member_profileForm" name="form1" method="post" action="ajax.aspx?oper=ajaxChangeProfile">
          <table align="center" id="studio">
            ...
            <tr>
              ...
              <td><input type="text" class="inputss" style="width:200px;" name="txtEmail" id="txtEmail" maxlength="50" />
                <span id="tipEmail" style="width: 200px"></span></td>
            </tr>
            <tr>
