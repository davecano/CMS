<%@ Page Language="C#" AutoEventWireup="True" Codebehind="maimai_orderlist.aspx.cs" Inherits="JumboTCMS.WebFile.User._maimai_orderlist" %>
...
function ajaxDel(ordernum)
{
	$.ajax({
		...
		url:		"ajax.aspx?oper=ajaxDeleteOrder&clienttime="+Math.random(),
		success:	function(d){
			switch (d.result)
			{
			case '0':
...
function ajaxPay(ordernum)
{
	$.ajax({
		...
		dataType:	"json",
		data:		"ordernum="+ordernum,
		url:		"ajax.aspx?oper=ajaxPayOrder&clienttime="+Math.random(),
		success:	function(d){
			switch (d.result)
			{
			case '0':
...
function ajaxFinish(ordernum)
{
	$.ajax({
		...
		dataType:	"json",
		data:		"ordernum="+ordernum,
		url:		"ajax.aspx?oper=ajaxFinishOrder&clienttime="+Math.random(),
		success:	function(d){
			switch (d.result)
			{
			case '0':
  ...
  <div id="main">
    ...
    <div id="mainarea">
      <table class="helptable mrg10T">
        <tr>
          <td><ul>
              <li>您现在有博币： <span class="u_points">0</span>，您可以选择直接支付或通过邮局汇款</li>
      ...
      <!--右侧列表-->
      <div id="list">
        <textarea class="template" id="tplList2" style="display:none">
	...
	<tr>
		...
		<td align="center">
			{#if $T.record.state == "0"}
			<a href="javascript:void(0);" onclick="JumboTCMS.Confirm('确定要作废该订单吗?', 'ajaxDel(\'{$T.record.ordernum}\')');">作废订单</a>
			{#else}
			<font color='#cccccc'>作废订单</font>
			{#/if}
			<br />
			{#if $T.record.state == "0"}
			<a href="javascript:void(0);" onclick="JumboTCMS.Confirm('确定要支付该订单吗?', 'ajaxPay(\'{$T.record.ordernum}\')');">立即支付</a>
			{#else}
			<font color='#cccccc'>立即支付</font>
			{#/if}
			<br />
			{#if $T.record.state == "2"}
			<a href="javascript:void(0);" onclick="JumboTCMS.Confirm('确定已收到商品了吗?', 'ajaxFinish(\'{$T.record.ordernum}\')');">确认收货</a>
			{#else}
