<%@ Page Language="C#" AutoEventWireup="True" Codebehind="maimai_cart.aspx.cs" Inherits="JumboTCMS.WebFile.User._maimai_cart" %>
...
$(document).ready(function(){
	...
	$("#txtTrueName")
		.formValidator({tipid:"tipTrueName",onshow:"2-8个汉字",onfocus:"2-8个汉字"})
		.InputValidator({min:4,max:16,onerror:"2-8个汉字"})
		.RegexValidator({regexp:"chinese",datatype:"enum",onerror:"请输入中文"});
	$("#txtAddress")
		.formValidator({empty:true,tipid:"tipAddress",onshow:"",onfocus:"请填写详细的地址"})
		.InputValidator({min:10,onerror: "填的信息不够详细!"});
	$("#txtZipCode").formValidator({tipid:"tipZipCode",onshow:"请输入邮编",onfocus:"请输入邮编,可以为空"}).InputValidator({min:6,max:6,onerror:"邮政编码为6位,请确认"}).RegexValidator({regexp:"zipcode",datatype:"enum",onerror:"格式不正确"});
	$("#txtMobileTel").formValidator({tipid:"tipMobileTel",onshow:"请输入你的手机号码",onfocus:"请输入你的手机号码"}).InputValidator({min:11,max:11,onerror:"必须是11位的,请确认"}).RegexValidator({regexp:"mobile",datatype:"enum",onerror:"不是正确的手机号码"});
	$("#chkAccept").formValidator({tipid:"tipAccept",onshow:"",onfocus:"请确认是否要提交"}).InputValidator({min:1,onerror:"请确认是否要提交"});
});
...
function ConfirmDel(id){
	JumboTCMS.Confirm("确定要从购物车里移除该商品吗?", "ajaxDel("+id+")");
}
...
function ajaxDel(id)
{
	$.ajax({
		...
		dataType:	"json",
		data:		"id="+id,
		url:		"ajax.aspx?oper=ajaxDelCart&clienttime="+Math.random(),
		success:	function(d){
			switch (d.result)
			{
			case '0':
...
function ajaxSetBuyCount(productid,buycount){
	...
	$.ajax({
		...
		url:		"ajax.aspx?oper=ajaxSetBuyCount&clienttime="+Math.random(),
