...
namespace JumboTCMS.WebFile.API._99bill
{
    public partial class _default : JumboTCMS.UI.FrontHtml
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //人民币网关账户号
            ///E:/swf/请登录快钱系统获取用户编号，用户编号后加01即为人民币网关账户号。
            merchantAcctId.Value = XmlTool.GetText("Root/merchantAcctId");
            //人民币网关密钥
            ///E:/swf/区分大小写.请与快钱联系索取
            string key = XmlTool.GetText("Root/key"); //人民币网关密钥
            XmlTool.Dispose();

            //字符集.固定选择值。可为空。
            ///E:/swf/只能选择1、2、3.
            ///E:/swf/1代表UTF-8; 2代表GBK; 3代表gb2312
            ///E:/swf/默认值为1
            ///E:/swf/如果在web.config文件中设置了编码方式，例如<globalization requestEncoding="utf-8" responseEncoding="utf-8"/>（如未设则默认为utf-8），
            ///E:/swf/那么，inputCharset的取值应与已设置的编码方式相一致
            inputCharset.Value = "1";


            //服务器接受支付结果的后台地址.与[pageUrl]不能同时为空。必须是绝对地址。
            ///E:/swf/快钱通过服务器连接的方式将交易结果发送到[bgUrl]对应的页面地址，在商户处理完成后输出的<result>如果为1，页面会转向到<redirecturl>对应的地址。
            ///E:/swf/如果快钱未接收到<redirecturl>对应的地址，快钱将把支付结果GET到[pageUrl]对应的页面。
            bgUrl.Value = site.Url + site.Dir + "api/99bill/receive.aspx";

            //网关版本.固定值
            ///E:/swf/快钱会根据版本号来调用对应的接口处理程序。
            ///E:/swf/本代码版本号固定为v2.0
            version.Value = "v2.0";
            ...
            //订单提交时间
            ///E:/swf/14位数字。年[4位]月[2位]日[2位]时[2位]分[2位]秒[2位]
            ///E:/swf/如；20080101010101
            ...
            //支付方式.固定选择值
            ///E:/swf/只能选择00、10、11、12、13、14
            ///E:/swf/00：组合支付（网关支付页面显示快钱支持的各种支付方式，推荐使用）10：银行卡支付（网关支付页面只显示银行卡支付）.11：电话银行支付（网关支付页面只显示电话支付）.12：快钱账户支付（网关支付页面只显示快钱账户支付）.13：线下支付（网关支付页面只显示线下支付方式）
            payType.Value = "00";


            //同一订单禁止重复提交标志
            ///E:/swf/固定选择值： 1、0
            ///E:/swf/1代表同一订单号只允许提交1次；0表示同一订单号在没有支付成功的前提下可重复提交多次。默认为0建议实物购物车结算类商户采用0；虚拟产品类商户采用1
            redoFlag.Value = "0";

            //快钱的合作伙伴的账户号
            ///E:/swf/如未和快钱签订代理合作协议，不需要填写本参数
            pid.Value = "";


            //生成加密签名串
            ///E:/swf/请务必按照如下顺序和规则组成加密串！
            String signMsgVal = "";
