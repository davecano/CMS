...
namespace JumboTCMS.WebFile.API._99bill
{
    public partial class receive : JumboTCMS.UI.FrontHtml
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //获取支付方式
            ///E:/swf/值为：10、11、12、13、14
            ///E:/swf/00：组合支付（网关支付页面显示快钱支持的各种支付方式，推荐使用）10：银行卡支付（网关支付页面只显示银行卡支付）.11：电话银行支付（网关支付页面只显示电话支付）.12：快钱账户支付（网关支付页面只显示快钱账户支付）.13：线下支付（网关支付页面只显示线下支付方式）.14：B2B支付（网关支付页面只显示B2B支付，但需要向快钱申请开通才能使用）
            ...
            //获取订单提交时间
            ///E:/swf/获取商户提交订单时的时间.14位数字。年[4位]月[2位]日[2位]时[2位]分[2位]秒[2位]
            ///E:/swf/如：20080101010101
            String orderTime = Request["orderTime"].ToString().Trim();

            //获取原始订单金额
            ///E:/swf/订单提交到快钱时的金额，单位为分。
            ///E:/swf/比方2 ，代表0.02元
            ...
            //获取银行交易号
            ///E:/swf/如果使用银行卡支付时，在银行的交易号。如不是通过银行支付，则为空
                ...
                switch (payResult)
                {
                    ...
                    case "10":
                        ...
                        */
                        ...
                        {
                            new JumboTCMS.DAL.Normal_UserNoticeDAL().SendNotite("在线支付", "您在线支付 " + Str2Int(payAmount) / 100 + " 元", userid, "recharge");
                            string username = new JumboTCMS.DAL.Normal_UserDAL().GetUserName(userid);
                            SendServiceNotice("会员在线支付", username + "在线支付 " + Str2Int(payAmount) / 100 + " 元", "1");
                            SendServiceNotice("会员在线支付", username + "在线支付 " + Str2Int(payAmount) / 100 + " 元", "3");
                        }
