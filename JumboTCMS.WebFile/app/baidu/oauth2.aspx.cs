                ...
                case "logined"://登陆后
                    ...
                    if (ob != null) //说明用户点击了授权，并跳回登陆界面来
                    {
                        ...
                        if (ob.Authorize(out account))//检测是否授权成功，并返回绑定的账号（具体是绑定ID还是用户名，你的选择）
                        {
                            ...
                            string _Email = "@";
                            JumboTCMS.Utils.Cookie.SetObj("oauthinfo_baidu", "{\"token\":\"" + ob.openID + "\",\"username\":\"" + _UserName + "\",\"email\":\"" + _Email + "\",\"birthday\":\"" + _Birthday + "\"}");
                            Response.Redirect(site.Dir + "passport/register_third.aspx?code=baidu");
...
授权失败</body></html>";
                ...
                default://登陆
                    ...
                    Response.Redirect(new JumboTCMS.OAuth2.BaiduOAuth().GetAuthorizeURL());
                    break;
            }
        }
    }
